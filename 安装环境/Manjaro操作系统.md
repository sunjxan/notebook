...
7. 在系统设置 - 开机与关机 -登陆屏幕（SDDM），选择“whitesur”使用；
8. 在桌面右键 - 配置桌面和壁纸 - 获取新壁纸，搜索“macos”，选择最多下载在前，安装壁纸，配置播放幻灯片；
9. 在桌面右键 - 配置桌面和壁纸 - 获取新壁纸，搜索“2560 x 1440”，选择最多下载在前，安装壁纸，在 `~/.local/share/wallpapers` 中选择壁纸复制，替换主题中的锁屏图片 `~/.local/share/wallpapers/WhiteSur/contents/images/2560x1440.png` ，原有图片重命名为 `backup.png` ；
10. 设置顶栏，右键编辑面板，点击配置系统托盘，将面板间距设置可变大小，将 `Manjaro设置管理器` 、 `News available!` 、 `Yakuake` 、 `蓝牙` 、 `剪贴板` 设置总是隐藏，配置数字时钟，日期自适应位置，不显示秒，时间显示24小时制；
11. 编辑终端Konsole，设置-显示工具栏 都取消，右键编辑当前方案，常规 设置命令为 `/bin/zsh` ，外观 - 配色方案和字体 选择微风，点编辑，勾选模糊背景，设置背景透明度为40%，外观 - 光标 设置为 I字型，闪烁已启用；

12. 在系统设置 - 显卡与显示器 - 显示器配置，设置全局缩放率为112.5%，重启电脑。
